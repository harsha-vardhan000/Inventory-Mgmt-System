...
class supplierClass:
    def __init__(self,root):
        ...
        #====searchFrame====
        #====options===
        lbl_cmb_search=Label(self.root,text="Invoice No.",bg="white",font=("times new roman",15))
        lbl_cmb_search.place(x=700,y=80)
        ...
        try:
            if self.var_sup_invoice.get()=="":
                ...
                if row==None:
                    ...
                    con.commit()
                    messagebox.showinfo("Success","Supplier Updated Successfuly",parent=self.root)
                    self.show()
        except Exception as ex:
            messagebox.showerror("Error",f"Error due to :{str(ex)}",parent=self.root)
